...
allowed-tools: WebSearch, WebFetch, Bash(npx tsx src/append-found.ts *), Bash(npx tsx src/discover-feeds.ts), Read, Write
---
...
```
npx tsx src/append-found.ts data/discovery/found.txt <url> [url...]
```

After adding URLs, run feed discovery:
```
npx tsx src/discover-feeds.ts
```
...
Tell the user:
1. Run `npm run ingest` to pull content from the new sources
2. Run `/discover` to find more sources iteratively
3. The newsletter pipeline is documented in `docs/newsletter-design.md`
